...
public class UsuarioEntity {
  ...
  @JoinTable(name = "tbl_usuario_grupo", inverseJoinColumns = { @JoinColumn(name = "id_usuario", table = "tbl_usuario_grupo") }, joinColumns = {
      @JoinColumn(name = "id_grupo", table = "tbl_usuario_grupo") })
  private List<GrupoEntity> grupos;
}
/*
 * create table
 *
 * users(username varchar_ignorecase(50) not null primary key,password
 * varchar_ignorecase(500) not null,enabled boolean not null); create table
 * authorities (username varchar_ignorecase(50) not null,authority
 * varchar_ignorecase(50) not null,constraint fk_authorities_users
 *
 * foreign key(username)
 *
 * references users(username)); create unique index ix_auth_username
 *
